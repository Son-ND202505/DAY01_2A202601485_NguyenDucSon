...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Dự phòng khi tiktoken chưa có, đang offline hoặc model không được biết.
...
def run_assistant(
    ...
    # Import trong hàm để các bài test có thể mock OpenAI chính xác.
    from openai import OpenAI
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    return "\n".join([make_row(headers), separator, *(make_row(row) for row in rows)])
